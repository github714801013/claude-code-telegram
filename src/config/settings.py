...
class Settings(BaseSettings):
    ...
    voice_provider: Literal["mistral", "openai", "custom"] = Field(
        "mistral",
        description="Voice transcription provider: 'mistral', 'openai', or 'custom'",
    )
    ...
    custom_voice_url: Optional[str] = Field(
        None, description="Custom voice provider URL (e.g., Xinference /v1/audio/transcriptions)"
    )
    custom_voice_api_key: Optional[SecretStr] = Field(
        None, description="Custom voice provider API key (optional)"
    )
    ...
    def validate_voice_provider(cls, v: Any) -> str:
        ...
        if provider not in {"mistral", "openai", "custom"}:
            raise ValueError("voice_provider must be one of ['mistral', 'openai', 'custom']")
        return provider
    ...
    def resolved_voice_model(self) -> str:
        ...
        if self.voice_provider == "openai":
            return "whisper-1"
        if self.voice_provider == "custom":
            return "SenseVoiceSmall" # default fallback for custom
